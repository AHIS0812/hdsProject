...
- 담당 1 인수인계: [`docs/04_담당1_인수인계.md`](docs/04_담당1_인수인계.md) — 담당 2 완료 · 담당 1이 끼울 지점·계약·할 일
- 시연 시나리오: [`docs/05_시연_시나리오.md`](docs/05_시연_시나리오.md) — 데모 순서·대사·Q&A·장애 대응. 실행은 `데모실행.bat`
- 평가: [`docs/03_평가결과.md`](docs/03_평가결과.md) — **1차(결정론적 변환기 기준선) 완료**, AI 파이프라인 연동 후 2차. 재현: `node scripts/run-eval.mjs`
...
- **캔버스 에디터** (`/`) — 요소 16종 배치/이동/리사이즈/스냅/undo·redo/줌/컨텍스트 툴바/단축키,
  다중 선택(Shift+클릭·마퀴 드래그) + 정렬·균등 분배 + 그룹화(Ctrl+G) + 우클릭 메뉴, 단축키 도움말(?)·첫 방문 온보딩,
