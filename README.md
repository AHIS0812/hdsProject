...
- **캔버스 에디터** (`/`) — 요소 16종 배치/이동/8방향 리사이즈/undo·redo(60)/줌(25~200%, 화면 배율 프리셋
  ...
  **연결 화살표**(여러 요소로 연결 가능, 설명 팝오버가 열려 있을 때만 캔버스에 표시 — 예: 조회 버튼 → 결과 그리드),
  ...
  "화면 생성" → 프로젝트의 모든 화면을 한 묶음으로 변환(결과 창 머리글의 ‹ › 또는 ←/→ 로 화면 이동,
  처음엔 편집 중이던 화면). 결과 모달 2탭 (화면 = 결과/동시 보기(내 스케치↔결과)·이미지 복사/저장(실제 화면 영역만,
...
- **여러 화면(페이지)** — 한 프로젝트에 화면 최대 50개. 캔버스 아래 화면 띠에서 추가·전환·복제·삭제(되돌리기 토스트)·
  끌어서 순서 바꾸기, PgUp/PgDn 이전·다음 화면. 띠는 하단 도구 모음의 **▤ 화면** 버튼이나 띠 위 가운데 손잡이로
  접고 펼 수 있고(접은 상태는 기억), 접어 둬도 마우스를 올리면 잠깐 펼쳐진다. 화면마다 유형·캔버스 크기·배경 이미지·되돌리기 기록이 따로 유지되고,
